...
public class GameController {
    ...
    public ResponseEntity<GameDto> createGame(HttpServletRequest request) {
        ...
        final GameDto gameDto = this.gameService.createGame(getUriProvider(request));
        return ResponseEntity.status(CREATED).body(gameDto);
    }


    // TODO: spec says this should be be a "PUT". Does this make sense? Stone distribution is
    //  not idempotent operation, shouldn't it be a POST?
    ...
    @PutMapping(path="/{gameId}/pits/{pitIndex}")
